...
class FimapElitesAdapter(BaseAdapter):
    ...
    def convert(self, data: Dict[str, Any]) -> Optional[UnifiedDungeonFormat]:
        try:
            ...
            # 修正：正确读取 cell->space_unit 映射
            cell_to_su = data.get("space_unit__per__cell", {})
            # cell_to_su: key=cell_id(str), value=su_id(int)
            # cell_locations: cell_id(str) -> {x, y}
            cell_locations = data.get("plan_graph", {}).get("location__per__vertex", {})
            cell_locations = {str(k): v for k, v in cell_locations.items()}
            # 统计每个space_unit包含的cell索引
            su_to_cells = {}
            for cell_id_str, su in cell_to_su.items():
                if su == -1:
                    continue
                su_to_cells.setdefault(su, []).append(cell_id_str)
            # 生成房间和走廊节点
            rooms = []
            corridors = []
            for su_id, su_type in type_per_space_unit.items():
                ...
                poly_points = [cell_locations.get(str(cid)) for cid in cell_ids if str(cid) in cell_locations]
                # 过滤掉None
                poly_points = [p for p in poly_points if p is not None]
