...
class AdvancedAnalytics(UnifiedChartGenerator):
    ...
    def perform_pca(self, data_matrix: np.ndarray, metric_names: List[str]) -> Dict:
        ...
        # 计算数据在主成分空间中的投影（transformed data）
        transformed_data = np.dot(data_scaled, eigenvectors)
        
        # 主成分的方向向量（components）就是特征向量
        components = eigenvectors.T  # 转置使每行代表一个主成分
    ...
    def perform_clustering(self, correlation_matrix: np.ndarray, metric_names: List[str]) -> Dict:
        ...
        for k in range(2, min(8, len(metric_names))):
            ...
            # 计算silhouette分数
            if len(set(clusters)) > 1 and len(set(clusters)) < len(metric_names):
                try:
                    # 简化的silhouette计算
                    silhouette_avg = self._calculate_silhouette_simple(distance_matrix, clusters)
                    silhouette_scores[str(k)] = {
    ...
    def _calculate_silhouette_simple(self, distance_matrix: np.ndarray, clusters: np.ndarray) -> float:
        """简化的silhouette系数计算"""
        n = len(clusters)
        silhouette_values = []
        ...
        for i in range(n):
            ...
            # 同一聚类内的平均距离 (a_i)
            same_cluster_indices = np.where(clusters == cluster_i)[0]
            ...
            for cluster_label in set(clusters):
                if cluster_label != cluster_i:
                    other_cluster_indices = np.where(clusters == cluster_label)[0]
                    if len(other_cluster_indices) > 0:
                        avg_dist = np.mean([distance_matrix[i, j] for j in other_cluster_indices])
