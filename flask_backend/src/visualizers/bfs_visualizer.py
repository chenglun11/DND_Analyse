...
class BFSVisualizer:
    ...
    def visualize_accessibility_analysis(self):
        ...
        plt.title('Accessibility Analysis', fontsize=14, fontweight='bold')
        plt.show()
    ...
    def visualize_path_diversity(self):
        ...
        if path_counts:
            ...
            stats_text = f'Average path diversity: {avg_path_diversity:.2f}\n'
            stats_text += f'Path count range: {min(path_counts)} - {max(path_counts)}\n'
            stats_text += f'Room pair count: {len(room_pairs)}'
        else:
            stats_text = 'No reachable paths'
        
        self.ax.text(0.02, 0.98, stats_text, transform=self.ax.transAxes,
                    fontsize=10, verticalalignment='top',
                    bbox=dict(boxstyle="round,pad=0.5", facecolor='white', alpha=0.8))
        
        plt.title('Path Diversity Analysis (Top 5 room pairs with most paths)', fontsize=14, fontweight='bold')
        plt.show()
